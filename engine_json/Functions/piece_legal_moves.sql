CREATE function [engine_json].[piece_legal_moves]
...
begin
	...
	from [engine_json].parse_pieces(@board_json) as bp
		join chess.colored_piece as cp on cp.id = bp.colored_piece_id
	where bp.id = @board_piece_id

	insert @result(col, row)
		select m.col, m.row 
		from engine_json.piece_potential_moves(@colored_piece, @piece_col, @piece_row, @attack_only) as m
		 where 
	...
	if @check_king = 1
		delete r
		from @result as r
		where engine_json.is_king_under_check(
												engine_json.make_move_json(
													  @board_json
													, @piece_col, @piece_row
													, r.col, r.row
												)
